...
##### indexwc index processing function
process_indexwc_indices<- function(dir,name){
  # get all files containing "indices"
  files <- list.files(
    path = dir,
    pattern = "^indices.*\\.csv$",
    full.names = TRUE
  ...
  # read files into named list
  indices_list <- set_names(files, basename(files)) |>
  ...
  indices_list <- imap(indices_list, function(df, nm) {
    ...
    df |>
      ...
      mutate(
        effort = nm,
        replicate = str_extract(nm, ".(?=\\.csv$)")
      ) |>
      mutate(
        effort = effort |>
          str_remove("^indices_") |>
          str_remove(paste0("^", name, "_")) |>
          str_remove("..\\.csv$")
      )
  })
  
  # bind all together
  indices_df <- bind_rows(indices_list)
  
  # write output
  write_csv(
    indices_df,
    file.path(dir, paste0(name, "_indices_df.csv"))
  )
